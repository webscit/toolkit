...
describe('Alert', () => {
  ...
  it('forwards className after scope anchor', async () => {
    ...
    await expect.element(screen.getByRole('alert')).toHaveClass('sct-alert my-class');
  });

  it('applies correct data-variant attribute', async () => {
    const screen = await render(<Alert variant="destructive">Error!</Alert>);
    await expect.element(screen.getByRole('alert')).toHaveAttribute('data-variant', 'destructive');
  });

  it('defaults to variant=default', async () => {
    const screen = await render(<Alert>Info</Alert>);
    await expect.element(screen.getByRole('alert')).toHaveAttribute('data-variant', 'default');
  });
