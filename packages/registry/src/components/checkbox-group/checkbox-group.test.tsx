...
describe("CheckboxGroup", () => {
  ...
  it("forwards className after scope anchor", async () => {
    const screen = await render(
      <CheckboxGroup className="my-class">children</CheckboxGroup>,
    );
    await expect
      .element(screen.getByRole("group"))
      .toHaveClass("sct-checkbox-group my-class");
  });

  it("renders legend when provided", async () => {
    const screen = await render(
      <CheckboxGroup legend="Fruits">children</CheckboxGroup>,
    );
  ...
  it("sets data-orientation attribute", async () => {
    const screen = await render(
      <CheckboxGroup orientation="horizontal">children</CheckboxGroup>,
    );
    await expect
      .element(screen.getByRole("group"))
