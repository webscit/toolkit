...
describe('Switch', () => {
  ...
  it('forwards className after scope anchor', async () => {
    ...
    await expect.element(screen.getByRole('switch')).toHaveClass('sct-switch my-class');
  });

  it('sets data-slot attribute', async () => {
    const screen = await render(<Switch />);
    await expect.element(screen.getByRole('switch')).toHaveAttribute('data-slot', 'switch');
  });
